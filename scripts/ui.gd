...
func close_walk_lsystem_card(cancel_recording: bool = true) -> void:
	walk_card_visible = false
	_remove_walk_lsystem_card()
	if _voice_list_has_only_empty_space():
		voice_list.add_child(_create_empty_lsystem_card())
	if cancel_recording:
		walk_recording_cancelled.emit()

func _on_generate_walk_close_pressed() -> void:
	close_walk_lsystem_card()
...
func _remove_walk_lsystem_card() -> void:
	...
	for child in voice_list.get_children():
		if child.name == "GenerateWalkCard":
			voice_list.remove_child(child)
			child.queue_free()

func _voice_list_has_only_empty_space() -> bool:
	if not voice_list:
		return false

	for child in voice_list.get_children():
		if child.name != "NoLSystemsCard" and child.name != "GenerateWalkCard":
			return false

	for child in voice_list.get_children():
		if child.name == "NoLSystemsCard":
			return false
